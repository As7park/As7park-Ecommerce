<script lang="ts">
	...
	let {
		...
		title = 'Sélection',
		accent = 'du moment',
		subtitle = 'Nos canettes du moment, à emporter avant le prochain départ.'
	}: {
		products: Product[];
		title?: string;
		accent?: string;
	...
	function categoryName(product: Product) {
		return product.categories?.[0]?.category?.name ?? 'AS7 Park';
	}
			...
			{#each products as product, i (product.id)}
				...
				>
					<div class="plate">
						...
						<div class="tag-wrap">
							<div class="price-tag"><span class="price">{formatPrice(product.price)}</span></div>
						</div>
...
<style lang="scss">
	// =========================================================================
	// AS7 PARK — RUNNER DE DALLES PRODUITS
